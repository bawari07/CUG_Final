...
//     }
//     if (userEmployeeCode) {
//       setEmployeeCode(userEmployeeCode);
//     } else {
//       console.error("Employee code not found in local storage");
//     }
//   }, []);
...
//           <ListItem
//             button
//             onClick={() => handleLinkClick("/simrequest")}
//             sx={{
//               justifyContent: isSidebarOpen ? "flex-start" : "center",
//               minHeight: "50px",
//               paddingLeft: isSidebarOpen ? "10px" : "30px",
...
//               <PeopleIcon />
//             </ListItemIcon>
//             {isSidebarOpen && <ListItemText primary="CUG allotment" />}
//           </ListItem>
//           {employeeCode === "01996" && (
//             <ListItem
//               button
//               onClick={() => handleLinkClick("/Regionalrequest")}
//               sx={{
//                 justifyContent: isSidebarOpen ? "flex-start" : "center",
//                 minHeight: "50px",
//                 paddingLeft: isSidebarOpen ? "10px" : "30px",
...
//               </ListItemIcon>
//               {isSidebarOpen && <ListItemText primary="Requests" />}
//             </ListItem>
//           )}
//           <ListItem
//             button
//             onClick={() => handleLinkClick("/SwapSim")}
//             sx={{
//               justifyContent: isSidebarOpen ? "flex-start" : "center",
//               minHeight: "50px",
//               paddingLeft: isSidebarOpen ? "10px" : "30px",
...
//             </ListItemIcon>
//             {isSidebarOpen && <ListItemText primary="Swap Sim Request" />}
//           </ListItem>
//           <ListItem
//             button
...
// NEW CODE FOR WHERE FAQ SECTIONS AND VOICES ARE ADDED .
...
const SidebarHr = () => {
  ...
  // new
  const isLimitedUser = employeeCode === "018405";
        ...
        <List>
          <ListItemIcon>
            <IconButton
              onClick={toggleSidebar}
              className="in_out"
              sx={{ margin: "8px", ml: "auto", color: "white" }}
            >
              <MenuIcon />
            </IconButton>
          </ListItemIcon>

          <ListItem button onClick={() => handleLinkClick("/HrDashboard")}> <ListItemIcon sx={{ color: "white" }}><DashboardIcon /></ListItemIcon> {isSidebarOpen && <ListItemText primary="Dashboard" />} </ListItem>
          {!isLimitedUser && <ListItem button onClick={() => handleLinkClick("/simrequest")}> <ListItemIcon sx={{ color: "white" }}><PeopleIcon /></ListItemIcon> {isSidebarOpen && <ListItemText primary="CUG allotment" />} </ListItem>}
          {!isLimitedUser && employeeCode === "01996" && <ListItem button onClick={() => handleLinkClick("/Regionalrequest")}> <ListItemIcon sx={{ color: "white" }}><i className="fs-4 bi-geo-alt"></i></ListItemIcon> {isSidebarOpen && <ListItemText primary="Requests " />} </ListItem>}
          {!isLimitedUser && <ListItem button onClick={() => handleLinkClick("/SwapSim")}> <ListItemIcon sx={{ color: "white" }}><i className="fs-4 bi-arrow-repeat"></i></ListItemIcon> {isSidebarOpen && <ListItemText primary="Swap Sim Request" />} </ListItem>}
          <ListItem button onClick={goToReviewRequestForm}> <ListItemIcon sx={{ color: "white" }}><TrackChangesIcon /></ListItemIcon> {isSidebarOpen && <ListItemText primary="Sim Record" />} </ListItem>
          <ListItem button onClick={handleFAQClick}> <ListItemIcon sx={{ color: "white" }}><i className="fs-4 bi-question-circle"></i></ListItemIcon> {isSidebarOpen && <ListItemText primary="FAQ" />} </ListItem>
          <ListItem button onClick={handleLogoutClick}> <ListItemIcon sx={{ color: "white" }}><i className="fs-4 bi-box-arrow-right"></i></ListItemIcon> {isSidebarOpen && <ListItemText primary="Logout" />} </ListItem>
        </List>
      </Drawer>
      {/* Modals */}
      {/* Logout Modal */}
      <Modal open={showModal} onClose={cancelLogout}><Box sx={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: 500, bgcolor: "background.paper", border: "2px solid #000", boxShadow: 24, p: 4 }}><h2>Confirm Logout</h2><p>Are you sure you want to log out?</p><Box sx={{ display: "flex", justifyContent: "flex-end", gap: 1 }}><Button onClick={cancelLogout}>Cancel</Button><Button variant="contained" color="primary" onClick={confirmLogout}>Logout</Button></Box></Box></Modal>
      {/* FAQ Modal */}
      <Modal open={showFAQModal} onClose={closeFAQModal}><Box sx={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: 600, maxHeight: "80vh", bgcolor: "background.paper", borderRadius: 2, boxShadow: 24, p: 4, overflowY: "auto" }}><Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}><h2>Frequently Asked Questions</h2><IconButton onClick={closeFAQModal}><i className="bi-x-lg"></i></IconButton></Box><Select value={language} onChange={handleLanguageChange} sx={{ mb: 2 }}><MenuItem value="en">English</MenuItem><MenuItem value="hi">Hindi</MenuItem></Select>{faqs[language].map((faq, index) => (<Accordion key={index} expanded={expanded === `panel${index}`} onChange={handleChange(`panel${index}`)} sx={{ mb: 2 }}><AccordionSummary expandIcon={<ExpandMoreIcon />}><Typography>{faq.question}</Typography><IconButton onClick={() => handleSpeak(faq.question)} sx={{ ml: 1 }}><VolumeUpIcon /></IconButton></AccordionSummary><AccordionDetails><Typography>{faq.answer}</Typography><IconButton onClick={() => handleSpeak(faq.answer)}><VolumeUpIcon /></IconButton><IconButton onClick={stopSpeech}><StopIcon /></IconButton></AccordionDetails></Accordion>))}</Box></Modal>
    </>
  );
};
