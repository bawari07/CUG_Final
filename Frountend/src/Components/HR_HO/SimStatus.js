...
function SimStatus() {
  ...
  useEffect(() => {
    async function fetchAllRequests() {
      try {
        const region = "Head Office";
        setLoadingRequests(true);
        const requestsResponse = await HRstatus({ region });
  ...
  const columns = [
    ...
    { field: "simAllocationType", headerName: "SIM Allocation Type", width: 200 },
  ...
  const filteredRows = filteredRequests
    .filter(
      (request) =>
        ...
        request.simAllocationType?.toLowerCase().includes(searchQuery.toLowerCase())
    )
    .map((request, index) => ({
      id: index + 1,
      ...
      <div className="sim-status-row">
        ...
        <div
          className={`sim-status-content ${
            isSidebarOpen ? "sim-status-content-expanded" : "sim-status-content-collapsed"
          }`}
        >
