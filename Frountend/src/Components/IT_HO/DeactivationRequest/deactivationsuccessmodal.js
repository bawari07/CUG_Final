...
const SuccessModal = ({ open, onClose, employeeName, requestType }) => {
  ...
  const getMessage = () => {
    if (requestType === "Deactivation") {
      return `Sim Deactivated successfully for ${employeeName}.`;
    } else if (requestType === "Suspension") {
      return `Sim Suspended successfully for ${employeeName}.`;
    }
  };
